...
func (r *agentExecutionRepository) GetByID(ctx context.Context, userID, executionID string) (*models.AgentExecution, error) { //nolint:lll
	...
	if err != nil {
		...
		logrus.WithFields(logrus.Fields{
			...
			"user_id":      userID,
			"error":        fmt.Sprintf("%+v", err),
		}).Error("Failed to get agent execution by ID")
	...
	if len(inputJSON) > 0 {
		if err := json.Unmarshal(inputJSON, &execution.Input); err != nil {
			logrus.WithFields(logrus.Fields{
				...
				"execution_id": executionID,
				"error":        fmt.Sprintf("%+v", err),
			}).Error("Failed to unmarshal input")
	...
	if len(artifactsJSON) > 0 {
		if err := json.Unmarshal(artifactsJSON, &execution.Artifacts); err != nil {
			logrus.WithFields(logrus.Fields{
				...
				"execution_id": executionID,
				"error":        fmt.Sprintf("%+v", err),
			}).Error("Failed to unmarshal artifacts")
...
func (r *agentExecutionRepository) countList(ctx context.Context, where squirrel.Sqlizer) (int, error) {
	...
	if err := r.db.QueryRowContext(ctx, query, args...).Scan(&totalCount); err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"method":  "List",
			"error":   fmt.Sprintf("%+v", err),
		}).Error("Failed to count agent executions")
...
func (r *agentExecutionRepository) queryList(
	...
	if err := rows.Err(); err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"method":  "List",
			"error":   fmt.Sprintf("%+v", err),
		}).Error("Error iterating agent execution rows")
...
func scanAgentExecutionListRows(rows *sql.Rows) ([]models.AgentExecution, error) {
	...
	for rows.Next() {
		...
		if scanErr := rows.Scan(&execution.ID, &execution.AgentID, &execution.UserID,
			...
			}).Error("Failed to scan agent execution row")
		...
		if len(inputJSON) > 0 {
			if jsonErr := json.Unmarshal(inputJSON, &execution.Input); jsonErr != nil {
				...
				}).Warn("Failed to unmarshal input, continuing with empty input")
...
func (r *agentExecutionRepository) GetByTaskID(ctx context.Context, userID, taskID string) (*models.AgentExecution, error) {
	...
	if err != nil {
		...
		logrus.WithFields(logrus.Fields{
			...
			"user_id": userID,
			"error":   fmt.Sprintf("%+v", err),
		}).Error("Failed to get agent execution by task ID")
...
func (r *agentExecutionRepository) UpdateTaskInfo(
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"error":         fmt.Sprintf("%+v", err),
		}).Error("Failed to update agent execution task info")
	...
	logrus.WithFields(logrus.Fields{
		...
		"context_id":    contextID,
		"current_state": currentState,
	}).Info("Agent execution task info updated")
...
func (r *agentExecutionRepository) UpdateStatus(ctx context.Context, executionID, status string) error {
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"status":       status,
			"error":        fmt.Sprintf("%+v", err),
		}).Error("Failed to update agent execution status")
	...
	if rowsAffected == 0 {
		...
		logrus.WithFields(logrus.Fields{
			...
			"execution_id": executionID,
			"status":       status,
		}).Debug("Execution not updated - may already be finalized")
		return nil
	}
	...
	logrus.WithFields(logrus.Fields{
		...
		"execution_id": executionID,
		"status":       status,
	}).Info("Agent execution status updated")
...
func (r *agentExecutionRepository) UpdateArtifacts(
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"execution_id": executionID,
			"error":        fmt.Sprintf("%+v", err),
		}).Error("Failed to marshal artifacts")
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"execution_id": executionID,
			"error":        fmt.Sprintf("%+v", err),
		}).Error("Failed to update agent execution artifacts")
	...
	}).Info("Agent execution artifacts updated")
...
func (r *agentExecutionRepository) GetByConversationID(
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"conversation_id": conversationID,
			"error":           fmt.Sprintf("%+v", err),
		}).Error("Failed to query executions by conversation")
	...
	for rows.Next() {
		...
		if scanErr != nil {
			logrus.WithFields(logrus.Fields{
				...
				"conversation_id": conversationID,
				"error":           fmt.Sprintf("%+v", scanErr),
			}).Error("Failed to scan execution row")
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"conversation_id": conversationID,
			"error":           fmt.Sprintf("%+v", err),
		}).Warn("Failed to get total count")
	...
	logrus.WithFields(logrus.Fields{
		...
		"has_more":        hasMore,
	}).Info("Retrieved executions by conversation")
...
func (r *agentExecutionRepository) GetFirstExecutionInConversation(ctx context.Context, userID, conversationID string) (*models.AgentExecution, error) {
	...
	logrus.WithFields(logrus.Fields{
		...
		"conversation_id": conversationID,
		"execution_id":    execution.ID,
	}).Info("Retrieved first execution in conversation")
...
func (r *agentExecutionRepository) ListConversations(
	...
	for rows.Next() {
		...
		if scanErr != nil {
			logrus.WithFields(logrus.Fields{
				...
				"agent_id": agentID,
				"error":    fmt.Sprintf("%+v", scanErr),
			}).Error("Failed to scan conversation row")
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"agent_id": agentID,
			"error":    fmt.Sprintf("%+v", err),
		}).Warn("Failed to get total conversation count")
	...
	logrus.WithFields(logrus.Fields{
		...
		"page":        page,
		"limit":       limit,
	}).Info("Retrieved conversation summaries")
