...
// TestIntegrationBlueprint_SlugConflict_NamesScope proves the #282 fix against
// real Postgres: a slug that is free in the target project but already used
// elsewhere in the same team collides on the team-wide blueprints_slug_team_id_key
...
func TestIntegrationBlueprint_SlugConflict_NamesScope(t *testing.T) {
	...
	newBlueprint := func(projectID, slug string) *models.Blueprint {
		now := time.Now()
	...
	require.NoError(t, repo.Create(ctx, newBlueprint(projectA, "dup")))

	// Cross-project, same team: free in project B per (project_id, slug), but the
	// team-wide key fires — the message must point at the team (#282).
	err = repo.Create(ctx, newBlueprint(projectB, "dup"))
	...
	err = repo.Create(ctx, newBlueprint(projectA, "dup"))
	require.Error(t, err)
