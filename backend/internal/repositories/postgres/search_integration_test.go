...
func TestSearchRepository_SearchKeyword_Integration(t *testing.T) {
	...
	t.Run("matches across types, honours status and ranks by relevance", func(t *testing.T) {
		...
		// A published prompt whose name+body are dense with the query terms ranks
		// highest; an active artifact mentioning them ranks lower; a memory matches too.
		strongPrompt := insertTestPrompt(t, user, team, project,
			"postgres full text search", "postgres full text search with tsvector and ts_rank", "published")
		...
		// Three matches (strong prompt, artifact, memory); the draft is filtered out
		// even though it repeats the term, and the frontend artifact doesn't match.
		assert.Equal(t, 3, total)
		require.Len(t, rows, 3)
		// Most relevant first (ORDER BY distance ASC == ts_rank DESC).
		assert.Equal(t, "prompt", rows[0].EntityType)
		assert.Equal(t, strongPrompt, rows[0].EntityID)
		// Score derives from 1 - distance and stays within [0,1).
		for _, r := range rows {
