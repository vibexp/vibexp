...
func setupCursorIDEDeleteTest(t *testing.T) (*cursorIDEHooksRepository, sqlmock.Sqlmock, *sql.DB) {
	mockDB, mock, err := sqlmock.New()
	require.NoError(t, err)

	db := &database.DB{DB: mockDB}
	repo := &cursorIDEHooksRepository{
		db: db,
	}
...
func TestCursorIDEHooksRepository_DeleteSession(t *testing.T) {
	tests := []struct {
		name          string
		userID        string
		sessionID     string
	...
	}{
		{
			...
			setupMock: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery(
					`SELECT EXISTS\(SELECT 1 FROM cursor_ide_hooks_payload WHERE user_id = \$1 AND session_id = \$2\)`,
				).
					WithArgs("user-123", "session-456").
					WillReturnRows(rows)

				// Mock DELETE operation
				mock.ExpectExec(`DELETE FROM cursor_ide_hooks_payload WHERE user_id = \$1 AND session_id = \$2`).
					WithArgs("user-123", "session-456").
...
func TestCursorIDEHooksRepository_DeleteSession_NilDB(t *testing.T) {
	repo := &cursorIDEHooksRepository{
		db: nil,
	}
