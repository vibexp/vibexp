...
func InitializeContainer(db *database.DB, cfg *config.Config, logger *slog.Logger) (Container, error) {
	...
	memoryServiceDeps := services.MemoryServiceDeps{
	...
	memoryServiceInterface := providers.ProvideMemoryService(memoryServiceDeps)
	...
	searchService := providers.ProvideSearchService(searchRepository, queryEmbedder, logger, searchSettingsResolver)
	llmService := providers.ProvideLLMService(modelProviderRepository, encryptionServiceInterface, cfg, logger)
	teamAISummarySettingsRepository := providers.ProvideTeamAISummarySettingsRepository(db)
	teamAISummarySettingsService := providers.ProvideTeamAISummarySettingsService(teamAISummarySettingsRepository, modelProviderRepository, authorizationServiceInterface, cfg, logger)
	searchSummaryServiceInterface := providers.ProvideSearchSummaryService(searchService, llmService, teamAISummarySettingsService, cfg, logger)
	...
	freshnessServiceDeps := services.FreshnessServiceDeps{
		Rules:     freshnessRuleRepository,
		Freshness: resourceFreshnessRepository,
		Settings:  teamFreshnessSettingsRepository,
		Audit:     freshnessAuditRepository,
		Schedules: scheduleRepository,
		Projects:  projectRepository,
		Authz:     authorizationServiceInterface,
		Logger:    logger,
	}
	freshnessServiceInterface := providers.ProvideFreshnessService(freshnessServiceDeps)
	...
	adminServiceInterface := providers.ProvideAdminService(adminRepository, userRepository, eventManager, freshnessServiceInterface)
	...
	gitHubAppServiceDeps := services.GitHubAppServiceDeps{
		InstallationRepo: gitHubInstallationRepository,
		ProjectRepo:      projectRepository,
		BlueprintRepo:    blueprintRepository,
	...
	gitHubAppServiceInterface := providers.ProvideGitHubAppService(gitHubAppServiceDeps)
	...
	wireContainer := &WireContainer{
		...
		searchSummaryService:         searchSummaryServiceInterface,
		aiSummaryAvailability:        teamAISummarySettingsService,
		...
		teamAISummarySettingsService: teamAISummarySettingsService,
		teamSettingsAuditService:     teamSettingsAuditServiceInterface,
