...
func TestMemoryService_UpdateSnapshotsOldContent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := services.NewMemoryService(repo, nil, permissiveAuthz(t), nil, logger, cvs, nil, nil, nil, nil)
...
func TestRestoreMemoryVersion_IsGated(t *testing.T) {
	...
	logger, _ := logtest.New()
	svc := services.NewMemoryService(repo, nil, authzMock, nil, logger, cvs, nil, nil, nil, nil)
