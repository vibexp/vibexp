...
func createTestMemoryService(repo repositories.MemoryRepository) *MemoryService {
	return NewMemoryService(repo, nil, allowAllAuthz{}, nil, func() *slog.Logger { l, _ := logtest.New(); return l }(), nil, nil, nil)
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service := NewMemoryService(
				...
				func() *slog.Logger { l, _ := logtest.New(); return l }(), nil, nil, nil)
