...
/**
 * Mock the chat execute/polling flow for a single agent happy path.
 *
 * A2A agents can't be exercised for real in E2E (no live agent-card URL), so
 * this reproduces the exact wire behaviour of a streaming agent that answers
 * with a direct message reply — the same shape our "A2A Dummy Test Agent"
 * produces:
 *   1. POST .../agents/{id}/execute        → an accepted execution (status
 *      "pending"), which makes the chat UI start event polling.
 *   2. GET  .../agents/executions/{id}/events → terminal "success" with zero
 *      events (a direct message reply carries no task/artifact events, so the
 *      real stream finalizes with total_events == 0).
 *   3. GET  .../agents/executions/{id}/status → the finalized execution whose
 *      artifacts carry the reply text the UI renders.
 *
 * Registered as three narrow routes; call this AFTER mockAgentsApi so these
 * win (Playwright runs the most-recently registered matching handler first).
 */
export async function mockAgentChatHappyPath(
  page: Page,
  options: {
    agentId: string
    reply: string
  ...
  const finalizedExecution = {
    ...
    ended_at: now,
    duration: 12,
    artifacts: [{ artifactId: 'response', parts: [{ text: reply }] }],
  }

  // POST .../agents/{id}/execute → accepted, still running (drives polling).
  await page.route(/\/agents\/[^/]+\/execute(\?|$)/, async route => {
  ...
  // GET .../agents/executions/{id}/events → terminal success, no events.
  await page.route(/\/agents\/executions\/[^/]+\/events(\?|$)/, async route => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  // GET .../agents/executions/{id}/status → finalized execution + reply.
  await page.route(/\/agents\/executions\/[^/]+\/status(\?|$)/, async route => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(finalizedExecution),
    })
  })
}
