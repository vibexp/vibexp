...
- **Dependency security runs in CI, not only pre-commit (#547).** `make frontend-audit`, `make backend-vulncheck` and `make backend-security` are CI steps, so a contributor without pre-commit installed no longer bypasses every dependency check. The two Go tools are **pinned in `ci.yml`** (`govulncheck@v1.6.0`, `gosec@v2.24.7`) for the same reason golangci-lint is — an unpinned bump turns a new upstream rule into a surprise red build on untouched code. Bump gosec deliberately, in its own PR, with the new findings triaged. `.github/dependabot.yml` (npm `/frontend`, gomod `/backend`, github-actions `/`) keeps the lockfiles moving weekly; its PRs carry `no-issue-needed` so `pr-linked-issue.yml` passes.
