...
- **API change flow.** Update the spec (`backend/openapi.yaml` / `backend/paths/**` / `backend/schemas/**`) -> merge to `main` -> the `@vibexp/api-client` publish is **automatic** -> bump the frontend dep. **Do NOT hand-publish the client** (no manual `workflow_dispatch`, no GitHub Release for a spec change): merging a spec change to `main` fires `.github/workflows/publish-api-client.yml`, which dispatches the api-client-js Publish workflow, and it **auto-minor-bumps** the npm version off the latest published release (every spec change = one new minor, breaking or not). Publishing by hand on top of that races the automation and can waste/duplicate a version. Only the frontend dependency bump remains manual (out of scope for the automation). Cross-repo trigger uses the "VibeXP Bot" GitHub App (installed on api-client-js, Actions: read/write; secrets `VIBEXP_BOT_APP_CLIENT_ID` / `VIBEXP_BOT_APP_PRIVATE_KEY` on this repo). Tracking: #303.
